...
function emptyProductLine() {
  return { _key: Math.random(), line_type: 'product', description: '', hsn_code: '', qty: '1', unit_price: '', discount_amount: '0', gst_rate: 18 };
}
      ...
      <View style={lc.topRow}>
        ...
        <TouchableOpacity style={[lc.searchBtn, { backgroundColor: colors.primary + '15' }]} onPress={onSearchProduct}>
          ...
          <Text style={[lc.searchBtnText, { color: colors.primary }]}>
            {line.description ? 'Change' : 'Search'}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={onRemove} style={lc.deleteBtn}>
          <Icon name="trash-outline" size={15} color="#EF4444" />
...
export default function NewQuotationScreen({ route, navigation }) {
  ...
  const [dateTarget,        setDateTarget]        = useState('quote');
  ...
  useEffect(() => {
    if (!isEdit) return;
    invoiceService.getQuotation(quoteId).then(q => {
      if (q.state === 'accepted' || q.state === 'cancelled') { navigation.goBack(); return; }
      setCustomer({ id: q.customer_id, name: q.customer_name, gstin: q.customer_gstin || '', address: q.customer_address || '' });
      setCustQuery(q.customer_name);
      setQuoteDate(q.quote_date);
      setValidUntil(q.valid_until || '');
      setPos(q.place_of_supply);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = isEdit
        ? await invoiceService.updateQuotation(quoteId, payload)
        : await invoiceService.createQuotation(payload);

      navigation.replace('QuotationDetail', { id: result.id });
    } catch (e) {
  ...
  return (
    <KeyboardAvoidingView
      ...
      {showDatePicker && (
        <DateTimePicker
          mode="date"
          value={new Date((dateTarget === 'quote' ? quoteDate : validUntil) || today)}
          onChange={(_, date) => {
            setShowDatePicker(false);
            if (!date) return;
            const iso = date.toISOString().slice(0, 10);
            if (dateTarget === 'quote') setQuoteDate(iso); else setValidUntil(iso);
          }}
        />
      )}
      ...
      <View style={[s.header, { backgroundColor: colors.surface, borderBottomColor: colors.divider }]}>
        ...
        <Text style={[s.title, { color: colors.text }]}>{isEdit ? 'Edit Quotation' : 'New Quotation'}</Text>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} keyboardDismissMode="on-drag">
        ...
        <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <View style={s.twoCol}>
            <View style={{ flex: 1 }}>
              <Text style={[s.fieldLabel, { color: colors.textLight }]}>Quote Date</Text>
              <TouchableOpacity style={[s.datePill, { borderColor: colors.border, backgroundColor: colors.background }]}
                onPress={() => { setDateTarget('quote'); setShowDatePicker(true); }}>
                <Icon name="calendar-outline" size={14} color={colors.textSecondary} />
                <Text style={[s.datePillText, { color: colors.text }]}>{fmtDate(quoteDate)}</Text>
              </TouchableOpacity>
            </View>
            <View style={{ flex: 1, marginLeft: 10 }}>
              <Text style={[s.fieldLabel, { color: colors.textLight }]}>Valid Until</Text>
              <TouchableOpacity style={[s.datePill, { borderColor: colors.border, backgroundColor: colors.background }]}
                onPress={() => { setDateTarget('valid'); setShowDatePicker(true); }}>
                <Icon name="calendar-outline" size={14} color={colors.textSecondary} />
                <Text style={[s.datePillText, { color: validUntil ? colors.text : colors.textLight }]}>
                  {validUntil ? fmtDate(validUntil) : 'Not set'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <TouchableOpacity style={[s.statePill, { borderColor: colors.border, backgroundColor: colors.background }]}
          ...
          <Text style={[s.fieldLabel, { color: colors.textLight, marginTop: 10 }]}>Payment Terms</Text>
          <TextInput
            style={[s.input, { color: colors.text, borderColor: colors.border, backgroundColor: colors.background }]}
        ...
        {/* Notes & Terms */}
        <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={[s.cardTitle, { color: colors.textLight }]}>NOTES & TERMS</Text>
          <TextInput
            style={[s.input, s.textArea, { color: colors.text, borderColor: colors.border, backgroundColor: colors.background }]}
